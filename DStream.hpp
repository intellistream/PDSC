...
struct Cell {
    ...
    Cell(int dimensions) : coordinates(dimensions, 0.0), density(0.0), timestamp(0.0) {}

    Cell(const std::vector<double>& coords, double density = 0.0, double timestamp = 0.0)
    ...
    double calcDistance(const Cell& cell) const {
        ...
        for (int i = 0; i < coordinates.size(); i++) {
            dist += (coordinates[i] - cell.coordinates[i]) * (coordinates[i] - cell.coordinates[i]);
        }
        return sqrt(dist);
    }
};
