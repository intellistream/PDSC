...
struct ClusterCell {
    ...
    ClusterCell(int dimensions) : seed(dimensions), density(0.0), dependent_distance(0.0), creation_time(0.0) {}
    ...
    double calcDistance(const Point& point) const {
        double dist = 0.0;
        for (int i = 0; i < point.coordinates.size(); i++) {
            dist += (point.coordinates[i] - seed.coordinates[i]) * (point.coordinates[i] - seed.coordinates[i]);
        }
        return sqrt(dist);
    }
};
