...
struct ClusteringFeature {
    ...
    ClusteringFeature(int dimensions) : linear_sum(dimensions, 0.0), squared_sum(dimensions, 0.0), n(0) {}
...
private:
    ...
    void insertCF(CFNodePtr &node, ClusteringFeature &cf, const Point &point) {
        if (node->isLeaf) {
            ...
            insertCF(node->children[closestIndex], node->entries[closestIndex], point);
        }
    }
